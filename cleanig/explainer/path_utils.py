...
class SBPPathGenerator(PathGenerator):
    """
    SBP path generator with proper uniform arc-length sampling.

    Key improvements over original version:
    1. Generates paths with uniform arc-length parameterization by default
    2. Optimized for speed while maintaining accuracy
    3. Ensures uniform distribution regardless of alpha value
    """
    def __init__(self, baseline_method, preprocess_fn, device, num_steps):
        super().__init__(baseline_method, preprocess_fn, device)
        self.num_steps = num_steps

    def get_paths(self, inputs, alpha_min, alpha_max, resample_uniform=True):
        """SBP path generation with uniform sampling."""
        ...
        baselines_flat = baselines.view(batch_size, -1)
        total_dims = inputs_flat.shape[1]

        # Sample alpha values
        if alpha_min == alpha_max:
            alphas = torch.full((batch_size,), alpha_min, device=self.device)
        else:
            alphas = torch.rand(batch_size, device=self.device) * (alpha_max - alpha_min) + alpha_min

        # Generate high-resolution initial paths for accurate arc-length
        n_initial_breaks = max(self.num_steps * 10, min(1000, total_dims * 10))

        # Vectorized stick-breaking for all samples
        uniform_samples = torch.rand(batch_size, total_dims, n_initial_breaks, device=self.device)

        # Compute betas using the transformation
        alphas_expanded = alphas.view(batch_size, 1, 1)

        # For numerical stability with small alpha
        small_alpha_mask = alphas < 0.1
        betas = torch.zeros_like(uniform_samples)

        if small_alpha_mask.any():
            log_u = torch.log(uniform_samples[small_alpha_mask] + 1e-10)
            betas[small_alpha_mask] = -torch.expm1(log_u / alphas_expanded[small_alpha_mask])

        if (~small_alpha_mask).any():
            betas[~small_alpha_mask] = 1 - torch.pow(
                uniform_samples[~small_alpha_mask],
                1.0 / alphas_expanded[~small_alpha_mask]
            )

        # Compute stick weights
        remaining_stick = torch.cumprod(1 - betas, dim=2)
        weights = betas.clone()
        weights[:, :, 1:] *= remaining_stick[:, :, :-1]

        # Generate random positions
        positions = torch.rand(batch_size, total_dims, n_initial_breaks, device=self.device)

        # Sort and compute CDF for each batch and dimension
        sorted_positions, sort_indices = torch.sort(positions, dim=2)
        sorted_weights = torch.gather(weights, 2, sort_indices)
        cumsum_weights = torch.cumsum(sorted_weights, dim=2)

        # Create dense evaluation grid
        n_eval = max(self.num_steps * 20, 500)
        t_eval = torch.linspace(0, 1, n_eval, device=self.device)
        t_eval = t_eval.unsqueeze(0).unsqueeze(0).expand(batch_size, total_dims, -1).contiguous()

        # Compute CDF at evaluation points
        indices = torch.searchsorted(sorted_positions, t_eval, right=True)

        # Pad and gather CDF values
        pad = torch.zeros((batch_size, total_dims, 1), device=self.device)
        padded_cumsum = torch.cat([pad, cumsum_weights], dim=2)

        # Create batch indices for gathering
        batch_idx = torch.arange(batch_size, device=self.device).view(batch_size, 1, 1)
        batch_idx = batch_idx.expand(batch_size, total_dims, n_eval)
        dim_idx = torch.arange(total_dims, device=self.device).view(1, total_dims, 1)
        dim_idx = dim_idx.expand(batch_size, total_dims, n_eval)

        cdf_values = padded_cumsum[batch_idx, dim_idx, indices]
        cdf_values[:, :, -1] = 1.0  # Ensure endpoint

        # Generate dense paths
        deltas = (inputs_flat - baselines_flat).unsqueeze(2)
        paths_flat = baselines_flat.unsqueeze(2) + cdf_values * deltas

        if resample_uniform:
            # Resample at uniform arc-length intervals
            resampled = self._resample(paths_flat, self.num_steps)
            paths_flat = resampled
        else:
            # Sample subset of points
            indices = torch.linspace(0, n_eval - 1, self.num_steps, dtype=torch.long)
            paths_flat = paths_flat[:, :, indices]

        # Reshape to original dimensions
        paths = paths_flat.transpose(1, 2).reshape(batch_size, -1, *input_dims)

        return paths

    def _resample(self, paths, n_points):
        """Arc-length resampling with improved accuracy."""
        batch_size, total_dims, n_current = paths.shape
        device = paths.device
        ...
        arc_lengths = torch.zeros(batch_size, n_current, device=device)
        arc_lengths[:, 1:] = torch.cumsum(distances, dim=1)

        # Normalize
        total_lengths = arc_lengths[:, -1:] + 1e-8
        normalized_arc_lengths = arc_lengths / total_lengths

        # Uniform sampling points
        uniform_t = torch.linspace(0, 1, n_points, device=device)
        uniform_t = uniform_t.unsqueeze(0).expand(batch_size, -1)

        # Find interpolation indices
        indices = torch.searchsorted(normalized_arc_lengths, uniform_t, right=False)
        indices = torch.clamp(indices, 1, n_current - 1)
        ...
        t_low = normalized_arc_lengths[batch_idx, idx_low]
        t_high = normalized_arc_lengths[batch_idx, idx_high]

        t_diff = torch.clamp(t_high - t_low, min=1e-8)
        w = torch.clamp((uniform_t - t_low) / t_diff, 0, 1).unsqueeze(1)

        # Gather and interpolate
        idx_low_exp = idx_low.unsqueeze(1).expand(-1, total_dims, -1)
        idx_high_exp = idx_high.unsqueeze(1).expand(-1, total_dims, -1)

        paths_low = paths.gather(2, idx_low_exp)
        paths_high = paths.gather(2, idx_high_exp)

        resampled = (1 - w) * paths_low + w * paths_high

        # Handle boundaries
        mask_start = (uniform_t == 0).unsqueeze(1)
        mask_end = (uniform_t == 1).unsqueeze(1)

        if mask_start.any():
            start_vals = paths[:, :, :1].expand(-1, -1, n_points)
            resampled = torch.where(mask_start, start_vals, resampled)

        if mask_end.any():
            end_vals = paths[:, :, -1:].expand(-1, -1, n_points)
            resampled = torch.where(mask_end, end_vals, resampled)
