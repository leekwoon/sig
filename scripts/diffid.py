...
def pipeline(args):
    ...
    save_npz = OmegaConf.select(args, "save_npz", default=True)
    eval_baseline_method = str(OmegaConf.select(args, "eval_baseline_method", default="mean"))
    reference_baseline_method = OmegaConf.select(args, "baseline_method", default=None)
    reference_baseline_label = (
        str(reference_baseline_method) if reference_baseline_method is not None else "n/a"
    )
    ...
    undo_preprocess_fn = partial(undo_preprocess, mean=args.dataset.mean, std=args.dataset.std)
    ...
    if args.explainer_name == 'ig':
        ...
        explainer = SpectralIGExplainer(
            ...
            spectral_mode=OmegaConf.select(args, "spectral_mode", default="svd"),
            channel_mode=OmegaConf.select(args, "channel_mode", default="per_channel"),
            gating_schedule=OmegaConf.select(args, "gating_schedule", default="linear"),
            gating_sigmoid_k=OmegaConf.select(args, "gating_sigmoid_k", default=12.0),
            wavelet_levels=OmegaConf.select(args, "wavelet_levels", default=4),
            laplacian_levels=OmegaConf.select(args, "laplacian_levels", default=4),
        )
        ...
        elif args.direction == 'deletion':
            line_types = ['deletion']
        elif args.direction == 'insertion':
            line_types = ['insertion']
        else:
            line_types = ['deletion', 'insertion']
        explainer = SAMPExplainer(
            model=model,
            device=args.device,
            exp_obj=args.exp_obj,
            preprocess_fn=preprocess_fn,
    ...
    if max_eval is not None:
        total_iters = (max_eval + args.dataset.batch_size - 1) // args.dataset.batch_size

    for images, labels in tqdm(val_loader, desc="Computing attributions", total=total_iters):
        ...
        score = compute_diffid_score(
            ...
            attributions,
            labels=labels,
            baseline_method=eval_baseline_method,
        ...
        if len(sample_images) < vis_count:
            ...
            sample_attributions.extend([attributions[i].cpu() for i in range(n_to_save)])
    ...
    if save_npz:
        images_np = undo_preprocess_fn(all_images).detach().cpu().numpy().transpose(0, 2, 3, 1)
    ...
    diffid_score, curves = compute_diffid_score(
        model, all_images, all_attributions, all_labels,
    ...
    results = {
        ...
        'reference_baseline_method': reference_baseline_label,
        'evaluation_baseline_method': eval_baseline_method,
    ...
    print(f"Reference baseline: {reference_baseline_label}")
    print(f"Evaluation baseline: {eval_baseline_method}")
    ...
    if vis_count > 0 and sample_images:
        ...
        for i in range(n_rows):
            img = undo_preprocess_fn(sample_images[i].unsqueeze(0)).squeeze(0).detach().cpu().numpy().transpose(1, 2, 0).clip(0, 1)
            axes[i, 0].imshow(img)
